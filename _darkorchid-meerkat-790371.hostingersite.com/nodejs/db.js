...
// PERSISTENCIA BLINDADA: Prioriza siempre la carpeta de AFUERA para evitar borrados al actualizar código
const localDir = path.join(__dirname, 'encuesta_central_data');
const globalDir = path.join(__dirname, '..', 'encuesta_central_data');

// Si existe la carpeta afuera, usamos esa. Si no, usamos la local.
const DATA_DIR = fs.existsSync(globalDir) ? globalDir : localDir;
...
function loadSchema() {
    ...
    const content = fs.readFileSync(SCHEMA_FILE, 'utf8').trim();
...
function loadEncuestas() {
    ...
    const content = fs.readFileSync(ENCUESTAS_FILE, 'utf8').trim();
    if (!content) return [];
    try {
        return JSON.parse(content);
    } catch (e) {
